...
func progressUpdate(steps:Int,goal:Int){
    let result:Double=Double(steps)/Double(goal)
    if(result >= 1.0){
 ...
 */
func pacing(from currentDistance:Double,to totalDistance:Double,from currentTime:Double,to goalTime:Double){
    let currentGoal=currentDistance/currentTime
    let totalGoal=totalDistance/goalTime
...
pacing(from:210,to:200,from:6,to:8)
